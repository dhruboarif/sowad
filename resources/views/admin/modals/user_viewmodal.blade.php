<div class="modal-size-default d-inline-block">
    ...
    <!-- Modal -->
    <div class="modal fade text-left" id="userviewModal{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="card-body">
                    <?php
                      $income_wallet=App\Models\CashWallet::where('user_id',$row->id)->get()->sum('bonus_amount');
                        $deposit_wallet=App\Models\AddMoney::where('user_id',$row->id)->where('status','approve')->get()->sum('amount');
                          $refferals = App\Models\User::where('sponsor',$row->id)->get()->count('id');

                          $carry=App\Models\User::where('id',$row->id)->first();
                          ...
                          $left_total=App\Models\User::where('id',$row->id)->first();
                          $right_total=App\Models\User::where('id',$row->id)->first();
                     ?>
                      <form action="#" method="post">
                        @csrf
                        <input type="hidden" name="id" value="">
                          <div class="form-group">
                              <label class="form-label" for="basic-default-name">Deposit Wallet(৳)</label>
                              <input readonly type="text" class="form-control"  value="{{$deposit_wallet}}" name="package_name" placeholder="Package name" />
                          </div>
                          <div class="form-group">
                              <label class="form-label" for="basic-default-email">Income Wallet(৳)</label>
                              <input type="number" readonly name="price" value="{{$income_wallet}}" class="form-control" placeholder="Enter Price ($)"/>
                          </div>
                          <div class="form-group">

...
@endphp
    <!-- Modal -->
    <div class="modal fade text-left" id="userviewModal{{isset($lev_three_right_l_l) ?$lev_three_right_l_l->id : ''}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="card-body">
                    <?php
                      $income_wallet=App\Models\CashWallet::where('user_id',$lev_three_right_l_l->id)->get()->sum('bonus_amount');
                        $deposit_wallet=App\Models\AddMoney::where('user_id',$lev_three_right_l_l->id)->where('status','approve')->get()->sum('amount');
                          $refferals = App\Models\User::where('sponsor',$lev_three_right_l_l->id)->get()->count('id');

                          $carry=App\Models\User::where('id',$lev_three_right_l_l->id)->first();
                          ...
                          $left_total=App\Models\User::where('id',$lev_three_right_l_l->id)->first();
                          $right_total=App\Models\User::where('id',$lev_three_right_l_l->id)->first();
                     ?>

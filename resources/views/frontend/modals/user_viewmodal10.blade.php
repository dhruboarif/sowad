...
@endphp
    <!-- Modal -->
    <div class="modal fade text-left" id="userviewModal{{isset($lev_three_left_r_r) ?$lev_three_left_r_r->id : ''}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="card-body">
                    <?php
                      $income_wallet=App\Models\CashWallet::where('user_id',$lev_three_left_r_r->id)->get()->sum('bonus_amount');
                        $deposit_wallet=App\Models\AddMoney::where('user_id',$lev_three_left_r_r->id)->where('status','approve')->get()->sum('amount');
                          $refferals = App\Models\User::where('sponsor',$lev_three_left_r_r->id)->get()->count('id');

                          $carry=App\Models\User::where('id',$lev_three_left_r_r->id)->first();
                          ...
                          $left_total=App\Models\User::where('id',$lev_three_left_r_r->id)->first();
                          $right_total=App\Models\User::where('id',$lev_three_left_r_r->id)->first();
                     ?>
